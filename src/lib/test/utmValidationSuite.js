...
export class UtmValidator {
  ...
  testUtmParameterParsing() {
    ...
    const testUrl = "https://budgetbeepro.com/quiz?utm_source=googleads&utm_campaign=22950333792&utm_content=id22950333792-771621153085&utm_medium=-22950333792";
    ...
    this.log(`Test Result: ${parseResult.passed ? 'PASS' : 'FAIL'}`, parseResult.details);
  ...
  testUtmNamingConventions() {
    ...
    const conventionChecks = {
      ...
      required_params_present: ['utm_source', 'utm_campaign'].every(param => testParams[param])
    };
  ...
  testDataLayerAccessibility() {
    ...
    const allAccessible = Object.values(accessibilityTests).every(test => test.passed);
  ...
  testUtmSourceValidation() {
    ...
    Object.entries(testSources).forEach(([source, expectedValid]) => {
      ...
      const allowedSources = [
        'convertkit', 'email', 'adwords', 'google', 'facebook',
        'instagram', 'twitter', 'linkedin', 'newsletter',
        'organic', 'direct', 'referral', 'googleads'
      ];
  ...
  testRealWorldCampaignUrl() {
    ...
    const campaignUrl = "https://budgetbeepro.com/quiz?utm_source=googleads&utm_campaign=22950333792&utm_content=id22950333792-771621153085&utm_medium=-22950333792";
    ...
    const campaignValidation = {
      ...
      content_has_campaign_reference: extractedParams.utm_content?.includes(extractedParams.utm_campaign || ''),
      medium_format_valid: extractedParams.utm_medium !== undefined
    };

    const campaignValid = Object.values(campaignValidation).every(check => check);
    ...
    const storageIntegrity = this.compareObjects(extractedParams, retrievedParams);
    ...
    this.log(`Test Result: ${campaignValid && storageIntegrity.passed ? 'PASS' : 'FAIL'}`);
  ...
  testSessionStorageAccess(params) {
    try {
      ...
      return { passed: Object.keys(retrieved).length === Object.keys(params).length, data: retrieved };
    } catch (error) {
  ...
  testDataLayerVariables(params) {
    try {
      ...
      const hasUtmData = lastEntry && lastEntry.utm_parameters &&
        Object.keys(lastEntry.utm_parameters).length === Object.keys(params).length;
  ...
  runAllTests() {
    ...
    this.log(`Test URL: https://budgetbeepro.com/quiz?utm_source=googleads&utm_campaign=22950333792&utm_content=id22950333792-771621153085&utm_medium=-22950333792`);
    ...
    if (passedCount === totalCount) {
      this.log("🎉 All tests passed! UTM parameter handling is working correctly.");
    } else {
...
if (typeof window !== 'undefined') {
  ...
  window.addEventListener('DOMContentLoaded', () => {
    console.log('UTM Validator loaded. Run: new UtmValidator().generateReport()');
  });
}
