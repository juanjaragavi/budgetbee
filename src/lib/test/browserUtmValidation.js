...
const testConfig = {
  ...
  testUrl: 'https://budgetbeepro.com/quiz?utm_source=googleads&utm_campaign=22950333792&utm_content=id22950333792-771621153085&utm_medium=-22950333792'
};
...
const urlParamTest = Object.keys(testConfig.expectedParams).every(key =>
  extractedUtms[key] === testConfig.expectedParams[key]
);
...
const sessionStorageTest = Object.keys(testConfig.expectedParams).every(key =>
  storedUtms[key] === testConfig.expectedParams[key]
);
...
const sourceValidation = allowedSources.includes((extractedUtms.utm_source || '').toLowerCase());
...
const dataLayerTest = dataLayerEntry &&
...
const pageContextTest = pageContext.pathname === '/quiz' &&
...
const formatValidation = {
  ...
  all_required_present: ['utm_source', 'utm_campaign'].every(key => extractedUtms[key])
};
...
const quizElements = {
  ...
  quizContainer: document.querySelector('[data-testid="quiz-container"]') ||
...
const quizFunctionalityTest = Object.values(quizElements).some(element =>
  element && (element.length > 0 || element.tagName)
);
...
const analyticsTest = analyticsReadiness.hasDataLayer &&
...
if (testResults.passed === testResults.total) {
  ...
  console.log('✅ The page can accept and process Google Ads campaign URLs properly.');
...
console.log('📋 To view saved results: console.log(window.utmValidationResults)');
