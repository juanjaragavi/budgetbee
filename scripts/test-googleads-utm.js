...
global.window = {
  location: {
    ...
    search:
      "?utm_source=googleads&utm_campaign=22950333792&utm_content=id22950333792-771621153085&utm_medium=-22950333792",
    pathname: "/quiz",
...
const parseTest = Object.keys(expectedParams).every(
  (key) => extractedParams[key] === expectedParams[key],
);
...
const storageTest =
  JSON.stringify(retrievedParams) === JSON.stringify(extractedParams);
...
const dataLayerTest =
  dataLayerEntry.event === "utm_parameters_loaded" &&
...
const urlTest =
  Object.keys(originalParams).every(
    (key) => originalParams[key] === constructedParams[key],
  ) &&
...
const tests = [
  ...
  { name: "Source Validation", result: sourceValidation },
  {
    ...
    result: Object.values(formatAnalysis).every((a) => a.valid),
  },
